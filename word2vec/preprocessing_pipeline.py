...
def one_line(input_path, output_path):

    """takes an input file and outputs a file containing one line
...
def remove_nextline(input_path, output_path):
    with open(input_path) as inp, open(output_path, 'w') as out:
        for line in inp:
            line = re.sub('\n',' ', line)
            out.write(line)

def add_to_all(input_path, output_path):
    with open(input_path) as inp, open(output_path, 'a') as out:
        for line in inp:
            out.write(line+'\n')
