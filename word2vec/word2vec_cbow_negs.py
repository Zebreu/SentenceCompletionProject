...
logger = logging.getLogger("word2vec_mod")
...
try:
    ...
    from word2vec_inner_cbow_negs import train_sentence, FAST_VERSION
except:
    ...
    def train_sentence(model, sentence, alpha, work=None, neu1=None): #mod
        ...
        for pos, word in enumerate(sentence):
            ...
            #reduced_window = 0  # `b` in the original word2vec code
...
class Word2Vec(utils.SaveLoad):
    ...
    """
    def __init__(self, sentences=None, size=100, neg_samples = 5, alpha=0.025, window=5, min_count=5, seed=1, workers=1, min_alpha=0.0001):
        """
        ...
        self.random_numbers = zeros(1000*self.neg_samples,dtype = uint32)
    ...
    def build_table(self):
        ...
        table_size_real = self.table_size + 0.
        vocab_size = len(self.vocab)
        self.table = zeros((self.table_size),dtype = uint32)
        train_words_pow = 0
        for i in xrange(vocab_size):
            cur_word = self.index2word[i]
            train_words_pow += pow(self.vocab[cur_word].count, power)
        train_words_pow += 0. #just to make sure this is represented as a real number
        i = 0
        cur_word = self.index2word[i]
        d1 = pow(self.vocab[cur_word].count,power) / train_words_pow
        for a in xrange(self.table_size):
            self.table[a] = i
            if a / table_size_real > d1:
                i += 1
                cur_word = self.index2word[i]
                d1 += pow(self.vocab[cur_word].count,power) / train_words_pow
            if i >= vocab_size:
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100):
        ...
        def worker_train():
            ...
            neu1 = matutils.zeros_aligned(self.layer1_size, dtype=REAL)  # each thread must have its own work memory
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    infile = sys.argv[1]
    from gensim.models.word2vec_mod import Word2Vec  # avoid referencing __main__ in pickle
